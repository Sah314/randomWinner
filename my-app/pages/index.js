...
const startGame = async()=>{
  try {
    ...
    const contract = new Contract(RANDOM_GAME_NFT_CONTRACT_ADDRESS,abi,signer);
    setLoading(true);
...
const checkIfGameStarted = async()=>{
  ...
  try {
    ...
    const contract = new Contract(RANDOM_GAME_NFT_CONTRACT_ADDRESS,abi,provider);
    const _gameStarted = await contract.gameStarted();
    const _gameArray = await subgraphQuery(FETCH_CREATED_GAME());
    //console.log(_gameArray.data.games);
    const _game = _gameArray.data.games[0];
    let _logs = [];
    if(_gameStarted){
...
const renderButton=()=>{
  ...
  if(gameStarted){
    if(players.length === maxPlayers){
      return (<button className={styles.button} disabled>Choosing Winner....</button>);
    }
    else{
      return(
        <div>
...
return(
  <div>
    <Head>
      ...
      <meta name="description" content="Random-game-dapp"/>
